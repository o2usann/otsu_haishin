...
def load_log():
    """points.json から log 配列を取り出す（なければ空）"""
    if not DATA.exists():
        return []
    try:
...
def filter_daily(log, today: date):
    out = []
    for e in log:
        ts = e.get("ts") or e.get("time")  # 互換
        if not ts:
            continue
        try:
            dt = parse_ts(ts).astimezone(JST)
        except Exception:
            continue
        if dt.date() == today:
            out.append(e)
...
def chart_page(title, totals, active_key):
    # 並びは pt 降順
    ...
    values_js = json.dumps(values, ensure_ascii=False)
...
def main():
    ...
    # index は月間へリダイレクト（好みで daily にしてもOK）
    index = """<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="UTF-8" />
